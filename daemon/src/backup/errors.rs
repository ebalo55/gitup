    ...
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            BackupError::CannotAddZipEntry(e) => {
                write!(f, "The backup file could not be added to the zip archive: {}", e)
            }
            BackupError::CannotWriteToZipArchive(e) => {
                write!(f, "The backup file could not be written to the zip archive: {}", e)
            }
            ...
            BackupError::NotEnoughSpace => {
                write!(f, "All providers failed or the storage providers do not have enough space to store the backup(s)")
            }
        }
    }
}
